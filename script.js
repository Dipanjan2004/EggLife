document.addEventListener('DOMContentLoaded', () => {
    ...
    [productModal, wtbModal].filter(Boolean).forEach(m => {
    ...
    if (backToTop || stickyCta) {
        const onScroll = () => {
            if (stickyCta && heroEl) {
                stickyCta.classList.toggle('is-visible', heroEl.getBoundingClientRect().bottom < 0);
            }
            if (backToTop) {
                const threshold = view2El ? view2El.getBoundingClientRect().bottom : -1;
                const show = view2El ? threshold < 0 : window.scrollY > 400;
    ...
    // FAQ accordion (details/summary works natively; this closes siblings)
